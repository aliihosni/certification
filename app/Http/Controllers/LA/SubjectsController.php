...
class SubjectsController extends Controller
{
    ...
    public function getsubject($id){
            ...
            $questions = DB::table('questions')->whereNull('deleted_at')->where('subject', $id)->get();


            foreach ($questions as $question){
                $respons = DB::table('responses')->whereNull('deleted_at')->where('question', $question->id)->get();
                $res[$question->id] = $respons;
            }
            //
            $data = [
                'id'=>$id,
            ...
            $pdf = PDF::loadView('la.subjects.subject', $data);
	...
	public function dtajax()
	{
		...
		for($i=0; $i < count($data->data); $i++) {
			...
			if($this->show_action) {
                ...
                $output .= '<a href="'.url(config('laraadmin.adminRoute') . '/subjects/getsubject/'.$data->data[$i][0]).'" class="btn btn-success btn-xs" style="display:inline;padding:2px 5px 3px 5px;"><i class="fa fa-file-pdf-o"></i></a>';
