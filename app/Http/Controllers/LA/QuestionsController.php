...
class QuestionsController extends Controller
{
    ...
    public function excel(){
        return view('la.questions.file_import_export',[]);
    }
    ...
    public function importFileIntoDB(Request $request){
        if($request->hasFile('sample_file')){
            ...
            if($data->count()){
                foreach ($data as $key => $value) {
                    $arr[] = ['content' => $value->content, 'subject' => $value->subject];
                }
                if(!empty($arr)){
        ...
        echo "<script>alert(\"Request data does not have any files to import.\")</script>";
        //redirect()->route(config('laraadmin.adminRoute') . '.questions.index');

        dd('Request data does not have any files to import.');
    }
    public function downloadExcelFile($type){
        $questions = Question::get()->toArray();
        return \Excel::create('Questions', function($excel) use ($questions) {
    ...
    public function modelExcelFile($type){
        $questions = array('content', 'subject');
        return \Excel::create('Question_model_to_fill', function($excel) use ($questions) {
            $excel->sheet('Questions', function($sheet) use ($questions)
            {
                $sheet->fromArray($questions);
            });
